...
'''
def calc_gw(temp_mus, temp_deltas, newMs, newBetas, newNyus, newWs):
    com = 1.0
    for k in range(3):
        #w = ss.wishart(df=newNyus[k], scale=newWs[k])
        w = ss.wishart(df=newNyus[k], scale=newWs[k])
        com *= normal_dist(temp_mus[k], newMs[k], temp_deltas[k], 3)
        #* w.pdf(temp_deltas[k])
    return com
'''
...
def main():
    ...
    while diff > 1000.0 and iter_c < 30:
        ...
        #newWs = [(Ws0.I + c1 + skxx[k] - newBetas[k] * np.dot(np.matrix(newMs[k]).T, np.matrix(newMs[k]))).I for k in range(K)]
